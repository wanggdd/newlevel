...
$where = array('m.enter_member'=>$user_user_id);
if($_GET['start_date']){
...
foreach($record_list as $key=>$item){
    ...
    $datas[] = array($item['user_name'],$nick_name,$item['mobile'],$item['enter_member']
    ,$out_time,$item['promote_money'],$status,$enter_time);
}
