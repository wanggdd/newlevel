...
//{$this->CurrentUrl}={$this->TotalPages}
    public function GetPagerContent() {
        $str = '';
        $str.= "<span class=\"page-number\">";
        //首页 上一页
        if($this->pageIndex==1)
        {
            $str .="<a href='javascript:void(0)' class='disabled' title='首页'>首页</a> "."\n";
            //$str .="<a href='javascript:void(0)' class='tips' title='上一页'>上一页</a> "."\n"."\n";
        }else
        {
            $str .="<a href='{$this->pageUrl}=1'  title='首页'>首页</a> "."\n";
            $str .="<a href='{$this->pageUrl}=".($this->pageIndex-1)."' data-page='".($this->pageIndex-1)."'>上一页</a> ";
        }
